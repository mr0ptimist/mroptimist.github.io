...
> 来源：Brian Karis, "A Deep Dive into Nanite Virtualized Geometry", GDC 2022 / SIGGRAPH 2021
...
- **Nanite 交互**：启用 Nanite 的 StaticMeshComponent 仍然是 Primitive，但不走传统 `FMeshPass` 绘制路径，而是被收集进 Nanite 自身的渲染 Pass
...
> 来源：Brian Karis, "A Deep Dive into Nanite Virtualized Geometry", GDC 2022
...
- **保守光栅化与深度 Only Pass**：Cluster 渲染管线使用保守光栅化技术和深度 Only Pass 减少需要完整材质着色的像素数
- **材质排序**：Material Pass 中像素按材质 ID 分组，提升缓存一致性

> 来源：Epic Games, UE 官方文档 "Nanite Virtualized Geometry"
> 来源：Brian Karis, GDC 2022 Nanite 技术演讲

---

## 光栅垃圾箱（Raster Bin）

Nanite 在光栅化阶段将 Cluster 按材质特性分桶（Bin），不同桶走不同的光栅化路径。源码中定义在 `NaniteRenderContext.h`：

```cpp
enum ERasterBin : uint8
{
    RASTER_BIN_OPAQUE,    // 标准不透明几何
    RASTER_BIN_MASKED,    // Alpha Test / 遮罩几何
    RASTER_BIN_COMPUTE,   // 需要计算光栅化的几何（WPO、曲面细分等）
    RASTER_BIN_COUNT
};
```
...
**启用方式**：
```
// 材质中勾选 Compute Nanite WPO
// 或 CVar
r.Nanite.ComputeWPO 1
```
...
VisBuffer 为全屏 R32_UINT 纹理（实际使用 64 bit，两个通道），每个像素编码：
...
| 来源 | 类型 | 链接 |
|------|------|------|
| Brian Karis, "A Deep Dive into Nanite Virtualized Geometry" | GDC 2022 演讲 | [Epic Developer Community](https://dev.epicgames.com/) |
| Epic Games, "Nanite Virtualized Geometry" | SIGGRAPH 2021 课程 | [SIGGRAPH 2021](https://advances.realtimerendering.com/s2021/) |
| Arseny Kapoulkine (zeux), "Nanite: GPU-driven rendering" | 技术博客 | [zeux.io](https://zeux.io/) |
| Epic Games, "Nanite Virtualized Geometry" | UE 官方文档 | [dev.epicgames.com](https://dev.epicgames.com/documentation/en-us/unrealEngine/nanite-virtualized-geometry-in-unreal-engine) |
| UE5 源码 `NaniteDataDecode.ush` | 引擎源码 | [GitHub - EpicGames/UnrealEngine](https://github.com/EpicGames/UnrealEngine) |
| UE5 源码 `NaniteStructs.h` | 引擎源码 | 同上 |
| UE5 源码 `NaniteMaterialShader.cpp` | 引擎源码 | 同上 |
| UE5 源码 `NaniteComputeRasterizer.usf` | 引擎源码 | 同上 |
| UE5 源码 `NaniteRenderContext.h` | 引擎源码 | 同上 |
